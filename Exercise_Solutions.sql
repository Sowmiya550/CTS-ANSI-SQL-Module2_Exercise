...
WHERE user_id NOT IN (
    ...
    FROM Registrations
    WHERE registration_date >= date('now','-90 day')
);
...
SELECT registration_date,
       COUNT(*) AS new_users
FROM Users
WHERE registration_date >= date('now','-7 day')
...
SELECT *
FROM Users u
WHERE u.registration_date >= date('now','-30 day')
AND NOT EXISTS (
    SELECT 1
...
SQLite version:
SELECT e.title,
       ROUND(AVG((julianday(s.end_time) - julianday(s.start_time))
       * 24 * 60),2) AS avg_duration_minutes
FROM Events e
JOIN Sessions s
ON e.event_id = s.event_id
GROUP BY e.event_id,e.title;
